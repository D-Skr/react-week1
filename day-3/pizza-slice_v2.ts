...
function drawMediumPizzaSlice(length: number, width: number): string {
    let mediumPizza = " ";
    let randtopping = Math.floor(Math.random() * (width-1)); 
    for(let i = 0; i < width; i++)
    {
        mediumPizza += '-';
    }
...
function drawAnyPizzaSlice(length: number, width:number): string {
    if(length <= 1 && width <= 1)
    {
        return drawSmallPizzaSlice(length, width);
    }
    else if(width >= 2)
    {
       return drawMediumPizzaSlice(length, width);
    }
    else
    {
        return drawLargePizzaSlice(length, width);
    }
...
console.log(drawAnyPizzaSlice(1, 2));